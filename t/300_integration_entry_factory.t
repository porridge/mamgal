...
lives_ok(sub { $e = MaMGal::EntryFactory->create_entry_for('td/symlink_to_empty_file') },"EF creates entry from symlink to empty file");
isa_ok($e, 'MaMGal::Entry::NonPicture',							"expected entry is a NonPicture");
is($e->name, 'symlink_to_empty_file',							"file name matches");

lives_ok(sub { $e = MaMGal::EntryFactory->create_entry_for('td/empty') },		"EF creates entry from empty dir");
isa_ok($e, 'MaMGal::Entry::Dir',								"expected entry is a Dir");
is($e->name, 'empty',									"file name matches");

lives_ok(sub { $e = MaMGal::EntryFactory->create_entry_for('td/symlink_to_empty') },	"EF creates entry from symlink to empty dir");
isa_ok($e, 'MaMGal::Entry::Dir',								"expected entry is a Dir");
is($e->name, 'symlink_to_empty',							"file name matches");

lives_ok(sub { $e = MaMGal::EntryFactory->create_entry_for('td/symlink_broken') },	"EF creates entry from broken symlink");
isa_ok($e, 'MaMGal::Entry::BrokenSymlink',						"expected entry is a BrokenSymlink");
is($e->name, 'symlink_broken',								"file name matches");

lives_ok(sub { $e = MaMGal::EntryFactory->create_entry_for('td/one_pic/a1.png') },	"EF creates entry from a picture");
isa_ok($e, 'MaMGal::Entry::Picture::Static',							"expected entry is a Picture::Static");
is($e->name, 'a1.png',									"file name matches");

lives_ok(sub { $e = MaMGal::EntryFactory->create_entry_for('td/symlink_pic.png') },	"EF creates entry from a symlink to picture");
isa_ok($e, 'MaMGal::Entry::Picture::Static',						"expected entry is a Picture::Static");
is($e->name, 'symlink_pic.png',								"file name matches");

lives_ok(sub { $e = MaMGal::EntryFactory->create_entry_for('td/symlink_pic_noext') },	"EF creates entry from a secret symlink to picture");
isa_ok($e, 'MaMGal::Entry::NonPicture',							"expected entry is a NonPicture");
is($e->name, 'symlink_pic_noext',							"file name matches");

dies_ok(sub { MaMGal::EntryFactory->create_entry_for('td/non-existant') },		"EF dies on nonexistant arg");

# some corner cases:
my $rootdir;
lives_ok(sub { $rootdir = MaMGal::EntryFactory->create_entry_for('/') },                 "EF creates entry for '/'");
...
my $cwd;
lives_ok(sub { $cwd = MaMGal::EntryFactory->create_entry_for('.') },                     "EF creates entry for '.'");
